...
var select = function(objects, fn) {
...
function Mobible() {
    ...
    self.language_filter = function(objects) {
        ...
        return select(objects, function(obj) {
            return obj.language.language_code == im.get_user_answer('language');
        });
    };
    ...
    self.add_creator('todays_story_intro', function(state_name, im) {
        ...
        p.add_callback(function(verses) {
            var cp = self.get_contact(im);
            cp.add_callback(function(result) {
                var contact = result.contact;
                var next_verse_index;
                if(contact['extras-mobible-last-journey'] != '' + journey_id) {
                    next_verse_index = 1;
                } else {
                    next_verse_index = parseInt(
                        contact['extras-mobible-last-verse-index'] || 0, 10) + 1;
                }
                return select(verses, function(verse) {
                    return verse.index == next_verse_index;
                })[0];
            });
            return cp;
        });
        p.add_callback(function(verse) {
            return new EndState(
                ...
                {
                    on_enter: function() {
                        var smsp = self.update_contact_fields(im, {
                            'mobible-last-journey': '' + verse.journey.id,
                            'mobible-last-verse-index': '' + verse.index
                        });

                        smsp.add_callback(
                            self.make_send_sms_function(im, im.user_addr, verse.context));
                        smsp.add_callback(
                            self.make_send_sms_function(im, im.user_addr, verse.verse));
                        return smsp;
                    }
                });
        });
