...
function Mobible() {
    ...
    self.add_creator('language', function(state_name, im) {
        ...
        p.add_callback(function(result) {
            if(result.created) next_state = 'introduction';
            else next_state = 'select_discovery_journey';
        });
    ...
    self.sms_introduction = function(im) {
        ...
        p.add_callback(function(introductions) {
            ...
            introductions.forEach(function(intro) {
                if(intro.language_code == im.get_user_answer('language')) {
                    sms.add_callback((function() {
                        return self.send_sms(im, im.user_addr, intro.text);
                    })());
                }
            });
            sms.callback();
    ...
    self.add_state(new EndState(
        ...
        {
            on_enter: function() {
                return self.sms_introduction(im);
            }
        }
    ));
