...
function Mobible() {
    ...
    // self.add_state(new EndState(
    //     "discovery_journey1_part1",
    //     "You will now receive the next verse.",
    //     "discovery_journey1_part2",
    //     {
    //         on_enter: function() {
    //             return self.send_sms(im, im.user_addr, self.stories[1].text);
    //         }
    //     }
    // ));
    ...
    //     {
    //         on_enter: function() {
    //             return self.send_sms(im, im.user_addr, self.stories[2].text);
    //         }
    //     }
    // ));
    ...
    self.add_state(new EndState(
        ...
        {
            on_enter: function() {
                forwards = im.get_user_answer('share_via_sms');
                if(forwards) {
                    var p = new Promise();
                    forwards.split(",").forEach(function(contact_number) {
                        var sms_text = self.stories[0].text;
                        var msisdn = contact_number.trim();
                        sms = self.send_sms(im, msisdn, sms_text);
                        p.add_callback(sms);
                    });
                    return p;
                }
