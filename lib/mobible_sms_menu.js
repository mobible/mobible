...
var MobibleSMSMenu = function () {
  ...
  self.add_creator('start', function (state_name, im) {
    ...
    p.add_callback(function (result) {
      ...
      var items = [
        new Choice('add_friend_name', 'Add friends to your group'),
      ...
      if(!self.has_registered(contact)) {
        items.unshift(new Choice('name', 'Signup for Mobible'));
      }
  ...
  self.add_state(new ChoiceState(
    ...
    'Do you want to add another friend?',
    [
      new Choice('yes', 'Yes'),
      new Choice('no', 'No')
    ]
  ...
  self.add_state(new EndState(
    'end',
    [
      'You have added your friend!',
      'Dial *120*8864*1252# to learn how',
      'to do your DBS and send the verse',
      'you will read to your friends!'
    ].join(' '),
    'start'
  ));
    ...
    p.add_callback(function(result) {
      ...
      if(contact.name && contact.surname) {
        ...
        return new ChoiceState(
          ...
          ('You need to sign up for Mobible ' +
           'before you can share verses. '),
          [
            new Choice('name', 'Sign up for Mobible.'),
            new Choice('start', 'Return to main menu.')
          ]
        );
      }
      ...
      on_enter: function() {
        ...
        p.add_callback(function (group) {
          ...
          up.add_callback(function(result) {
            ...
            var sms_p = self.send_sms(
              im.user_addr, im.get_user_answer('_cached_verse_content'));
            sms_p.add_callback(function (result) {
  ...
  self.add_state(new ChoiceState(
    ...
    [
      new Choice('unsubscribe_group', 'Leave this group.'),
      new Choice('start', 'Go back to the start menu.')
    ]
  ));
